...
In short: items 1, 2, 4, 5, 6 are landed in code and tested. Item 3 has the daemon trait skeleton + binary wired; the descriptor-chain handler is the one remaining code wedge that requires a real vhost-user-master to validate. Item 7 (real microVM smoke) is gated on item 3's last wedge plus operator hardware setup. The runbook at `docs/runbooks/raft-block-microvm-smoke.md` is the canonical procedure.

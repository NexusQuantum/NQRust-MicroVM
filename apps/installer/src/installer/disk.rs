...
pub fn run_disk_install(
    ...
    logs.push(LogEntry::info(
        "Copying live system to target disk (this may take 5-10 minutes)...",
    ));
...
/// Helper to send a log entry through the channel
fn send_log(tx: &Sender<LogEntry>, entry: LogEntry) {
    let _ = tx.send(entry);
}

/// Air-gapped disk installation with real-time progress updates via channel
/// This version sends logs immediately instead of collecting them
pub fn run_disk_install_with_sender(
    disk: &DiskInfo,
    hostname: &str,
    root_password: &str,
    bundle_path: &Path,
    ...
    // Step 1: Partition disk
    send_log(&tx, LogEntry::info("Partitioning disk..."));
    let mut partition_logs = Vec::new();
    let layout = partition_disk(disk, &mut partition_logs)?;
    for log in partition_logs {
        send_log(&tx, log);
    }
    send_log(&tx, LogEntry::success("Disk partitioned"));

    // Step 2: Mount partitions
    send_log(&tx, LogEntry::info("Mounting partitions..."));
    let mut mount_logs = Vec::new();
    mount_partitions(&layout, &mut mount_logs)?;
    for log in mount_logs {
        send_log(&tx, log);
    }
    send_log(&tx, LogEntry::success("Partitions mounted"));

    // Step 3: Copy live system using rsync
    send_log(
        &tx,
        LogEntry::info("Copying live system to target disk (this may take 5-10 minutes)..."),
    );
    ...
    match rsync_result {
        Ok(output) if output.status.success() || output.status.code() == Some(24) => {
            send_log(&tx, LogEntry::success("Live system copied to target disk"));
        }
        Ok(output) => {
            let stderr = String::from_utf8_lossy(&output.stderr);
            send_log(&tx, LogEntry::error(format!("rsync failed: {}", stderr)));
            return Err(anyhow!("Failed to copy live system"));
        }
        Err(e) => {
            send_log(&tx, LogEntry::error(format!("rsync error: {}", e)));
            return Err(e);
        }
    }

    // Create essential directories
    send_log(&tx, LogEntry::info("Creating essential directories..."));
    for dir in &["dev", "proc", "sys", "tmp", "run", "mnt", "media"] {
        let path = format!("{}/{}", TARGET_MOUNT, dir);
        let _ = fs::create_dir_all(&path);
    }
    let _ = run_sudo("chmod", &["1777", &format!("{}/tmp", TARGET_MOUNT)]);
    send_log(&tx, LogEntry::success("Directories created"));

    // Mount virtual filesystems
    send_log(&tx, LogEntry::info("Mounting virtual filesystems..."));
    run_sudo(
        "mount",
        &["--bind", "/dev", &format!("{}/dev", TARGET_MOUNT)],
    )?;
    ...
    send_log(&tx, LogEntry::success("Virtual filesystems mounted"));

    // Generate fstab
    send_log(&tx, LogEntry::info("Generating /etc/fstab..."));
    let root_uuid = get_uuid(&layout.root_part)?;
    let mut fstab = "# /etc/fstab - generated by NQRust installer\n".to_string();
    fstab.push_str(&format!(
        "UUID={}  /  ext4  errors=remount-ro  0  1\n",
    ...
    fs::write(format!("{}/etc/fstab", TARGET_MOUNT), fstab)?;
    send_log(&tx, LogEntry::success("fstab generated"));

    // Set hostname
    send_log(
        &tx,
        LogEntry::info(format!("Setting hostname to '{}'...", hostname)),
    );
    fs::write(
        format!("{}/etc/hostname", TARGET_MOUNT),
        format!("{}\n", hostname),
    ...
    send_log(&tx, LogEntry::success("Hostname configured"));

    // Remove live-boot hooks
    send_log(&tx, LogEntry::info("Removing live-boot hooks..."));
    let _ = fs::remove_dir_all(format!("{}/lib/live", TARGET_MOUNT));
    let _ = run_sudo(
        "chroot",
        &[
    ...
    send_log(&tx, LogEntry::success("Live-boot hooks removed"));

    // Configure systemd as init
    send_log(&tx, LogEntry::info("Configuring systemd as init system..."));
    let _ = fs::remove_file(format!("{}/sbin/init", TARGET_MOUNT));
    let _ = std::os::unix::fs::symlink(
        "/lib/systemd/systemd",
        format!("{}/sbin/init", TARGET_MOUNT),
    );
    send_log(&tx, LogEntry::success("Systemd configured"));

    // Set root password
    send_log(&tx, LogEntry::info("Setting root password..."));
    chroot_run(&format!("echo 'root:{}' | chpasswd", root_password))?;
    send_log(&tx, LogEntry::success("Root password set"));

    // Create nqrust user
    send_log(&tx, LogEntry::info("Creating nqrust user..."));
    let _ = chroot_run("useradd --system --no-create-home --shell /usr/sbin/nologin nqrust");
    send_log(&tx, LogEntry::success("User created"));

    // Configure SSH
    send_log(&tx, LogEntry::info("Configuring SSH..."));
    let _ = chroot_run(
        "sed -i 's/^#*PasswordAuthentication.*/PasswordAuthentication yes/' /etc/ssh/sshd_config",
    );
    let _ = chroot_run("sed -i 's/^#*PermitRootLogin.*/PermitRootLogin yes/' /etc/ssh/sshd_config");
    send_log(&tx, LogEntry::success("SSH configured"));

    // Enable services
    send_log(&tx, LogEntry::info("Enabling services..."));
    let _ = chroot_run("systemctl enable ssh");
    let _ = chroot_run("systemctl enable NetworkManager");
    let _ = chroot_run("systemctl enable postgresql");
    send_log(&tx, LogEntry::success("Services enabled"));
    ...
    send_log(&tx, LogEntry::info("Regenerating initramfs..."));
    let _ = chroot_run("update-initramfs -u -k all");
    send_log(&tx, LogEntry::success("Initramfs regenerated"));

    // Install bootloader
    send_log(&tx, LogEntry::info("Installing bootloader..."));
    let mut bootloader_logs = Vec::new();
    install_bootloader(disk, &mut bootloader_logs)?;
    for log in bootloader_logs {
        send_log(&tx, log);
    }
    send_log(&tx, LogEntry::success("Bootloader installed"));

    // Install NQRust components from bundle
    send_log(&tx, LogEntry::info("Installing NQRust components..."));
    let mut nqrust_logs = Vec::new();
    install_nqrust_components(bundle_path, &mut nqrust_logs)?;
    for log in nqrust_logs {
        send_log(&tx, log);
    }
    send_log(&tx, LogEntry::success("NQRust components installed"));

    // Create systemd services for NQRust
    send_log(&tx, LogEntry::info("Creating NQRust systemd services..."));
    let mut service_logs = Vec::new();
    create_nqrust_services(&mut service_logs)?;
    for log in service_logs {
        send_log(&tx, log);
    }
    send_log(&tx, LogEntry::success("Systemd services created"));

    // Cleanup
    send_log(&tx, LogEntry::info("Cleaning up..."));
    let mut cleanup_logs = Vec::new();
    unmount_all(&mut cleanup_logs)?;
    for log in cleanup_logs {
        send_log(&tx, log);
    }
    send_log(&tx, LogEntry::success("Cleanup complete"));

    send_log(&tx, LogEntry::success("Installation complete!"));
    send_log(
        &tx,
        LogEntry::info("You can now reboot and remove the installation media."),
    );
    send_log(
        &tx,
        LogEntry::info(format!("Login as root with password: {}", root_password)),
    );
...
fn install_nqrust_components(bundle_path: &Path, logs: &mut Vec<LogEntry>) -> Result<()> {
    ...
    // Copy binaries (NQRust + Firecracker)
    if bin_src.exists() {
        for binary in &[
            "nqr-manager",
            "nqr-agent",
        ...
        for binary in &["firecracker", "jailer"] {
            let src = format!("{}/{}", bin_dst, binary);
            let dst = format!("{}/usr/local/bin/{}", TARGET_MOUNT, binary);
        ...
        logs.push(LogEntry::warning(format!(
            "Bundle bin directory not found: {:?}",
            bin_src
        )));
    }

    // Copy rootfs images (runtimes like bun, node, python, alpine, etc.)
    let rootfs_src = bundle_path.join("images/rootfs");
    let rootfs_dst = format!("{}/srv/fc/images", TARGET_MOUNT);
    if rootfs_src.exists() {
        logs.push(LogEntry::info("Copying rootfs images (runtimes)..."));
        fs::create_dir_all(&rootfs_dst)?;
        let _ = run_sudo(
            "cp",
            &[
                "-r",
                &format!("{}/*", rootfs_src.display()),
                &format!("{}/", rootfs_dst),
            ],
        );

        // List what was copied
        if let Ok(entries) = fs::read_dir(&rootfs_src) {
            for entry in entries.flatten() {
                if let Some(name) = entry.file_name().to_str() {
                    logs.push(LogEntry::success(format!("Copied rootfs: {}", name)));
                }
            }
        }
    } else {
        logs.push(LogEntry::warning("No rootfs images found in bundle"));
    }

    // Copy kernel images
    let kernels_src = bundle_path.join("images/kernel");
    let kernels_dst = format!("{}/srv/fc/kernels", TARGET_MOUNT);
    if kernels_src.exists() {
        logs.push(LogEntry::info("Copying kernel images..."));
        fs::create_dir_all(&kernels_dst)?;
        let _ = run_sudo(
            "cp",
            &[
                "-r",
        ...
        if let Ok(entries) = fs::read_dir(&kernels_src) {
            for entry in entries.flatten() {
                if let Some(name) = entry.file_name().to_str() {
                    logs.push(LogEntry::success(format!("Copied kernel: {}", name)));
                }
            }
        }
    } else {
        logs.push(LogEntry::warning("No kernel images found in bundle"));
    }

    // Copy UI (pre-built Next.js app)
    let ui_src = bundle_path.join("ui");
    let ui_dst = format!("{}/opt/nqrust-microvm/ui", TARGET_MOUNT);
    if ui_src.exists() {
        logs.push(LogEntry::info("Copying Web UI..."));
        fs::create_dir_all(&ui_dst)?;
        let _ = run_sudo(
            "cp",
            &[
                "-r",
                &format!("{}/*", ui_src.display()),
                &format!("{}/", ui_dst),
            ],
        );
        logs.push(LogEntry::success("Web UI installed"));
    } else {
        logs.push(LogEntry::warning(
            "Web UI not found in bundle - UI service will not be available",
        ));
    }
...
Environment=RUST_LOG=info
Environment=AGENT_BIND=0.0.0.0:19090
...
"#;
    ...
    fs::write(
        format!("{}/etc/systemd/system/nqrust-ui.service", TARGET_MOUNT),
        ui_service,
    )?;

    // Create UI environment file
    let ui_env = r#"# NQRust-MicroVM UI Configuration
PORT=3000
...
"#;
    ...
    fs::create_dir_all(format!("{}/etc/nqrust-microvm", TARGET_MOUNT))?;
    fs::write(
        format!("{}/etc/nqrust-microvm/ui.env", TARGET_MOUNT),
        ui_env,
    )?;
...
usermod -aG kvm nqrust 2>/dev/null || true
...
# Enable NAT for VMs
iptables -t nat -A POSTROUTING -o $(ip route | grep default | awk '{print $5}') -j MASQUERADE 2>/dev/null || true

# Check if UI is installed and enable it
if [ -d "/opt/nqrust-microvm/ui" ] && [ -f "/opt/nqrust-microvm/ui/package.json" ]; then
    systemctl enable nqrust-ui.service
    systemctl start nqrust-ui.service
fi
...
"#;
    ...
    fs::write(
        format!(
            "{}/etc/systemd/system/nqrust-firstboot.service",
            TARGET_MOUNT
        ),
        firstboot_service,
    )?;

...
fn resolve_external_host(config: &InstallConfig) -> String {
    if config.network_mode == NetworkMode::Nat || config.network_mode == NetworkMode::Isolated {
        return "localhost".to_string();
    }
    // Bridged: try live interface detection
...
/// Generate a cryptographically-secure 32-byte random key, hex-encoded.
/// Used for SSO_ENCRYPTION_KEY (AES-256-GCM for OIDC client secrets at rest).
fn generate_sso_encryption_key() -> String {
...
fn generate_manager_config(config: &InstallConfig, db_password: &str) -> Result<Vec<LogEntry>> {
    ...
    // Resolve the externally-reachable host and derive SSO URLs from it.
    // For NAT/Isolated setups this is "localhost" (SSO then only works locally,
    // which is fine for single-host installs). For Bridged, we use the detected IP.
    // Admins who need a DNS name or HTTPS can override SSO_BASE_URL/SSO_FRONTEND_URL
    // by editing manager.env and restarting the service.
    let external_host = resolve_external_host(config);
    let sso_base_url = format!("http://{}:18080", external_host);
    let sso_frontend_url = format!("http://{}:3000", external_host);
    let sso_encryption_key = generate_sso_encryption_key();
...
# SSO_BASE_URL is the externally-reachable manager URL used as the redirect_uri
# base for OIDC/SAML flows. Must match EXACTLY what you register in your IdP.
# Edit these if you expose the manager via a DNS name or HTTPS.
SSO_BASE_URL={}
SSO_FRONTEND_URL={}
# 32-byte random key (hex) for AES-256-GCM encryption of OIDC client secrets
# stored in the database. Generated once at install; rotating it will invalidate
# all stored secrets.
SSO_ENCRYPTION_KEY={}
# Uncomment for dev environments with self-signed IdP certs:
# SSO_INSECURE_SKIP_TLS_VERIFY=true
...
"#,
    ...
    logs.push(LogEntry::info(format!(
        "SSO base URL: {} (edit manager.env if you need a different hostname)",
        sso_base_url
    )));
    logs.push(LogEntry::success(
        "Generated random SSO encryption key (32 bytes)",
    ));

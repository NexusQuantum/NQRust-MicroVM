...
pub async fn update_guest_ip(
    ...
    // Ignore 0.0.0.0 - this is used by the guest agent as a test payload for VM ID validation
    if req.guest_ip == "0.0.0.0" {
        tracing::debug!(vm_id = %id, "Ignoring 0.0.0.0 IP (test payload)");
        return Ok(Json(OkResponse::default()));
    }

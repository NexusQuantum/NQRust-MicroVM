...
/// Ensure global dnsmasq settings (like port=0) exist in a single file.
/// Per-network configs must NOT repeat global options — dnsmasq 2.90+ rejects duplicates.
async fn ensure_dnsmasq_globals() -> Result<()> {
    ...
    let globals_path = "/etc/dnsmasq.d/nqrust-globals.conf";
    if !std::path::Path::new(globals_path).exists() {
        tokio::fs::write(
            globals_path,
            "# Global NQRust dnsmasq settings (written once by agent)\n\
             # Disable DNS (DHCP only) to avoid systemd-resolved port 53 conflict.\n\
             port=0\n\
             bind-dynamic\n",
        )
        .await
        .context("failed to write dnsmasq globals config")?;
    }
    Ok(())
}
